...
RenderOp VulkanRenderGraph::bindDescriptorSets(std::shared_ptr<VulkanShader> shader) {
    return [&](VkCommandBuffer commandBuffer) {
        ...
        // NOTE:
        // Does shader->name, shader->bindPoint, etc get called at lambda creation time or lambda run time?
        ...
        for (auto set : descriptorManager->descriptors[shader->name]->getSets()) {
            vkCmdBindDescriptorSets(commandBuffer, shader->bindPoint, pipelines[shader->name]->pipelineLayout(), set.first, 1, &set.second,
                                    0, nullptr);
        }
    };
}
...
RenderOp VulkanRenderGraph::pushConstants(std::shared_ptr<VulkanShader> shader, void* data) {
    return [&](VkCommandBuffer commandBuffer) {
        vkCmdPushConstants(commandBuffer, pipelines[shader->name]->pipelineLayout(), shader->stageFlags, shader->pushConstantRange.offset,
                           shader->pushConstantRange.size, data);
    };
}
